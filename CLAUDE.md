...
Targets Vercel. `vercel.json` sets `outputDirectory: dist` and rewrites all paths to `index.html` (needed even though there's currently no client-side router, in case one is added later). Requires `VITE_SUPABASE_URL` and `VITE_SUPABASE_ANON_KEY` set in the Vercel project's environment variables (Vite inlines them at build time — adding them after a deploy requires a redeploy to take effect).
