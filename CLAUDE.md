...
- `src/main.js` — everything else: Leaflet map setup, `navigator.geolocation` handling, DOM wiring for the form/list, and keeping the map markers in sync with storage. State (current geolocation fix, `id -> L.Marker` map) is held in a single module-level `state` object rather than framework state; the map itself (`L.marker`) is the source of truth for what's rendered, and `storage.js` is the source of truth for what's persisted — `renderMarker`/`removeMarker` keep the two in sync on every save/delete.
